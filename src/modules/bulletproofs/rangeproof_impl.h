...
static int secp256k1_bulletproof_rangeproof_verify_impl(const secp256k1_callback *error_callback, secp256k1_scratch *scratch, const unsigned char* const* proof, const size_t n_proofs, const size_t plen, size_t nbits, const uint64_t* const* min_value, const secp256k1_ge* const* commitp, size_t n_commits, const secp256k1_ge *value_gen, const secp256k1_bulletproof_generators *gens, const unsigned char* const* extra_commit, size_t *extra_commit_len) {
    ...
    ecmult_data = (secp256k1_bulletproof_vfy_ecmult_context *)secp256k1_scratch_alloc(error_callback, scratch, n_proofs * sizeof(*ecmult_data));
    innp_ctx = (secp256k1_bulletproof_innerproduct_context *)secp256k1_scratch_alloc(error_callback, scratch, n_proofs * sizeof(*innp_ctx));
    ...
    ret = secp256k1_bulletproof_inner_product_verify_impl(error_callback, scratch, gens, nbits * n_commits, innp_ctx, n_proofs, plen - (64 + 128 + 1), same_generators);
    secp256k1_scratch_apply_checkpoint(error_callback, scratch, scratch_checkpoint);
    return ret;
}
...
static int secp256k1_bulletproof_rangeproof_prove_impl(
    ...
    *plen -= 64 + 128 + 1;
    ...
    if (secp256k1_bulletproof_inner_product_prove_impl(error_callback, scratch, &proof[64 + 128 + 1], plen, gens, &y, nbits * n_commits, secp256k1_bulletproof_abgh_callback, (void *) &abgh_data, commit) == 0) {
        return 0;
    }
    *plen += 64 + 128 + 1;
